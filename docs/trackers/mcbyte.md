...
| `cmc_downscale`                           | `2`               | Downscale factor applied to frames before camera motion compensation.                                                                    | Higher (`4`, `6`) is faster but produces coarser motion estimates; see [Performance and optimization](#performance-and-optimization).                                     |
...
| `minimum_mask_creation_frames`            | `3`               | Consecutive visible frames a confirmed tracklet needs before its mask is created (SAM prompt and Cutie `add_masks`).                     | Set to `1` for immediate mask creation; higher values defer mask work for short-lived tracks, leaving IoU-only association until a mask exists.                           |
