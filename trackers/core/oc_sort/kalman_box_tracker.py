...
class OCSORTKalmanBoxTracker(SORTKalmanBoxTracker):
    ...
    Args:
        bbox (NDArray[np.float64]): Initial bounding box.
        ocm_observation_history_size (int): Maximum number of past observations to store
            for OCM.
    """
    ...
    def _observation_centric_re_update(self, bbox: NDArray[np.float32]) -> None:
        ...
        Args:
            bbox (NDArray[np.float32]): The new observation that re-activated
                the track.
        """
