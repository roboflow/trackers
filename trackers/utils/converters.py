...
def xyxy_to_xcycsr(xyxy: np.ndarray) -> np.ndarray:
    ...
    Args:
        xyxy: A numpy array of shape `(4)` which corresponds to a bounding box
...
def xcycsr_to_xyxy(xcycsr: np.ndarray) -> np.ndarray:
    ...
    Args:
        xcycsr: A numpy array of shape `(4)` which corresponds to a centered bounding
            box in the form [x,y,s,r]
    Returns:
