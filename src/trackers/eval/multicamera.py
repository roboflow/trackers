...
"""AI City 2024 multi-camera evaluation on the world plane."""
...
from trackers.io.multicamera import _prepare_multicamera_files, load_scene_camera_map
...
class SceneMeanHOTA:
    ...
    The protocol averages four per-scene fields across scenes. Recall,
    precision, and detection counts have no benchmark-level definition under
    an unweighted mean, so they are absent rather than null.
    ...
    def from_dict(cls, data: dict[str, Any]) -> SceneMeanHOTA:
        ...
        """
        return cls(**{field: float(data[field]) for field in SCENE_MEAN_FIELDS})
    ...
    def to_dict(self) -> dict[str, float]:
        ...
        """
        return {field: float(getattr(self, field)) for field in SCENE_MEAN_FIELDS}


@dataclass
class MulticameraBenchmarkResult:
    """Result for an AI City 2024 multi-camera benchmark run.
    ...
    def table(self) -> str:
        ...
        rows: list[tuple[str, dict[str, float | int]]] = [
            (name, _sequence_row(self.scenes[name], SCENE_MEAN_FIELDS)) for name in sorted(self.scenes)
        ]
        aggregate: dict[str, float | int] = dict(self.aggregate.to_dict())
        rows.append((_AGGREGATE_LABEL, aggregate))
        return _format_metric_rows(rows, SCENE_MEAN_FIELDS, rule_before_last=True)
...
def evaluate_multicamera_scenes(
    ...
    *,
    ...
    An existing but empty prediction file scores the scene as zero.
...
def _scene_mean(scene_results: dict[str, SequenceResult]) -> SceneMeanHOTA:
    ...
    hota_results = [scene.HOTA for scene in scene_results.values() if scene.HOTA is not None]
    if not hota_results:
        raise ValueError("No HOTA results available to average.")

    return SceneMeanHOTA(
        **{field: float(np.mean([getattr(hota, field) for hota in hota_results])) for field in SCENE_MEAN_FIELDS}
    )
