...
| `reid_model`                              | Optional ReID model for appearance-based association.                         | Requires `frame` in :meth:`update`. When set, each track stores an EMA feature bank and BoT-SORT gated-min fusion is applied in the first association stage.                                                                           |
| `reid_emb_dist_threshold`                 | Appearance distance gate θ_emb for active tracks.                             | Default `0.25` (BoT-SORT paper). On MOT17 with a MOT-trained encoder, `0.2` often works better ([SCCAI 2025](https://www-sop.inria.fr/members/Francois.Bremond/Postscript/Tomasz__SCCAI_2025.pdf)). Increase to allow more appearance matches.                                                                                                                                                            |
| `reid_iou_dist_threshold`                 | IoU distance gate θ_iou before appearance is used for active tracks.          | Default `0.5` (IoU > 0.5). Increase to loosen proximity gating.                                                                                                                                                                        |
| `reid_gated_app_distance_scale`           | Appearance distance scale when the proximity gate passes.                     | Default `0.5`.                                                                                                                                                                                                                         |
| `reid_iou_dist_threshold_lost`            | IoU distance gate for lost tracks during ReID fusion.                         | Defaults to `reid_iou_dist_threshold`. Set to `1.0` to skip IoU gating for lost tracks.                                                                                                                                                |
| `reid_emb_dist_threshold_lost`            | Appearance distance gate for lost tracks and appearance-only recovery.        | Defaults to `reid_emb_dist_threshold`. Loosen for re-entry when appearance changes (distance, angle, partial visibility).                                                                                                              |
