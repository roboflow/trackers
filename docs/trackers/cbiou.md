...
<!-- BENCH-XREF copy-of: docs/benchmarking/results.md C-BIoU row in mot17-default/sportsmot-default/soccernet-default/dancetrack-default tables. Also duplicated in README.md (Algorithms table) — note docs/index.md's Algorithms table has NO C-BIoU row (only SORT/ByteTrack/OC-SORT/BoT-SORT), so C-BIoU changes don't touch index.md. Update results.md first, then mirror here. -->
...
=== "CLI"
    ...
    Run C-BIoU on a video without writing any Python. See the [CLI reference](../guides/cli.md) for every argument, including `--source 0` for a webcam or an `rtsp://` URL for a stream.
...
For BIoU mathematics and using `BIoU(buffer_ratio=...)` on other trackers, see [IoU variants](../guides/iou.md#biou). To tune hyperparameters with Optuna, see [Hyperparameter tuning](../guides/tune.md).
