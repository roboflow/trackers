...
def _valid_ground_truth_mask(frame_data: _MOTFrameData, config: MOTClassConfig) -> NDArray[np.bool_]:
    ...
    when it is marked for consideration (``conf != 0``) and belongs to a scored
    class. Distractor-class and ignored rows are excluded so they are never
    counted as false negatives.

    Args:
        frame_data: Detections for a single ground-truth frame.
        config: MOT class configuration specifying scored and distractor classes.

    Returns:
        Boolean array of shape `(N,)`, `True` for scored ground-truth rows.
    """
    classes = frame_data.classes
    if not config.scored_classes or len(classes) == 0:
        return np.zeros(len(classes), dtype=bool)
    class_mask = reduce(or_, (classes == class_id for class_id in config.scored_classes))
    return (frame_data.confidences != 0) & class_mask


def _distractor_ground_truth_mask(frame_data: _MOTFrameData, config: MOTClassConfig) -> NDArray[np.bool_]:
    ...
    """
    classes = frame_data.classes
    if not config.distractor_classes or len(classes) == 0:
        return np.zeros(len(classes), dtype=bool)
    return reduce(or_, (classes == class_id for class_id in config.distractor_classes))
...
def _build_id_mappings(
    ...
    for frame in range(1, num_frames + 1):
        if frame in ground_truth_data:
            valid_mask = _valid_ground_truth_mask(ground_truth_data[frame], config)
...
def _extract_ground_truth_frame(
    ...
    if frame in ground_truth_data:
        ...
        valid_mask = _valid_ground_truth_mask(frame_data, config)
        return (
            frame_data.boxes[valid_mask],
            frame_data.ids[valid_mask],
            frame_data.boxes,
            _distractor_ground_truth_mask(frame_data, config),
        )
...
def _prepare_mot_sequence(
    ...
    """
    config = resolve_mot_class_config(class_config)
    num_frames = _resolve_num_frames(ground_truth_data, tracker_data, num_frames)
    ground_truth_id_map, tracker_id_map = _build_id_mappings(ground_truth_data, tracker_data, num_frames, config)
    ...
    for frame in range(1, num_frames + 1):
        ground_truth_boxes, ground_truth_ids, all_boxes, distractor_mask = _extract_ground_truth_frame(
            ground_truth_data, frame, config
        )
