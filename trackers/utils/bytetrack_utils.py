...
def linear_assignment_with_cost_limit_scipy(
    ...
    if not np.isfinite(cost_limit):
        # Standard rectangular assignment; everything will be matched up to min(n_tracks, n_dets)
    ...
    for rr, cc in zip(r, c):
        # rr < n_tracks and cc >= n_dets  => track rr is assigned to a dummy column (unmatched track)
        # rr >= n_tracks and cc < n_dets  => detection cc is assigned to a dummy row (unmatched detection)
        ...
        if rr < n_tracks and cc < n_dets:
            ...
            # With this construction, costs > cost_limit should be dominated by "unmatch both" cost (=cost_limit),
            # but we enforce it explicitly anyway.
            if cost_matrix[rr, cc] <= cost_limit:
                ...
                print(
                    f"Warning: ignoring match with cost {cost_matrix[rr, cc]} > thresh {cost_limit}"
                )
