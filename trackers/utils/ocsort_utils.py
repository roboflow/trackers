...
def add_track_id_detections(
    ...
    Args:
        ...
        tracklets (Sequence[OCSORTTracklet]): List of OCSORTTracklet objects.
        matched_indices (list[tuple[int, int]]): List of matched (tracklet_idx, detection_idx) pairs.
    Returns:
...
def get_iou_matrix_between_boxes(
    ...
    Args:
        last_observations (np.ndarray): Last observed bounding boxes of tracks in the
            form [x1, y1, x2, y2].
        detection_boxes (np.ndarray): Detected bounding boxes in the
            form [x1, y1, x2, y2].
