...
**OSNet ~52.9 mAP / 78.7 Rank-1** and **FastReID SBS R50-ibn ~65.4 mAP / 85.1 Rank-1**.
...
| **OSNet / OSNet-AIN** (Zhou et al., ICCV'19; TorchReID)                 | "Omni-scale" residual blocks aggregate features across multiple receptive-field scales with a unified aggregation gate; extremely lightweight. *Tomasz's torchreid option.*                                        | OSNet (~2.2M)                          | **52.9 / 78.7**                     | Yes — TorchReID model zoo + HF (`kaiyangzhou/osnet`) | **Low** to consume / **Medium** clean-room               | **Best** (tiny, fast) |
